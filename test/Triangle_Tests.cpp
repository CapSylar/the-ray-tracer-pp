...
TEST_CASE("triangle tests")
{
    ...
    {
        REQUIRE(t.normal_at(Point3f(0,0.5f,0)) == t._n1);
        REQUIRE(t.normal_at(Point3f(-0.5f,0.75f,0)) == t._n1);
        REQUIRE(t.normal_at(Point3f(0.5f,0.25f,0)) == t._n1);
    }

    SECTION("intersecting a ray parallel to the triangle")
    {
        Ray ray ( Point3f(0,-1,-2) , Vec3f(0,1,0));
        t.intersect( ray , list );
...
//        const auto normal = t.normal_at( Point3f( 0.45f , 0.25f , 0 ) );
//
